...
- **日期**：`YYYY` / `YYYY-MM-DD` / `财年`；`1995-01-01`→as_of_year=1995。
  - **默认规则（F，日历与解析共用）**：仅年份 → 当年 `12-30`（注明「年初」→`01-01`）；年份+月份 → 该月**月底**（写明「月初」→该月 `1 日`）；「上旬/中旬/下旬」→`1 日/11 日/21 日`。
...
- 预计算：`snapshot(as_of_year, scope, value)`，覆盖 account×币种、entity、family:total。
- 口径：`as_of_year 状态 = 固定值 + 累计到 as_of_year 的年标记变更`（§PRD 6.3）。
- 日历游标：前端按所选年份读 `snapshot` 一次，无需实时计算。
- **全局日历游标（E）**：游标为全 App 级常驻（顶部固定），非仅 Dashboard；所有 人物/公司/财务/曲线 屏统一按 `as_of_year` 读取各自快照，拖动一次全屏联动，观看「截至所选日期节点」的状态。
...
| 模块 | 功能 | 关键章节 | 状态 |
|---|---|---|---|
| **工程骨架** | config 三环境 / db / alembic 迁移 / ingest CLI | §3–§4 | ⬜ |
| **Ingest** | detect 类别识别 + 解析器(bank/股票表/汇率/人物/时间线) + normalize | §6 | ⬜ |
| **Ingest** | 导入前冲突检测 hard-block（conflict.py，§11.4 + 金额/余额等） | §11.4 | ⬜ |
| **Phase1 摄入** | 初始资产建档(initial_asset) + 现金进余额 | §6.5 | ⬜ |
| **Phase1 摄入** | 收益文件(租/经营性房/祖产债券/开店/薪资)模块化挂账(income_stream) | §6.5/§6.3 | ⬜ |
| **Phase1 摄入** | 家庭支出(挂 Henri)/薪资各归各账户；2002 BEF/LUF/NLG 关池转 EUR | §6.5/§6.6 | ⬜ |
| **DDL** | entity/account(status/closed_on)/ledger_entry/income_stream/initial_asset/snapshot 等 | §5 | ⬜ |
| **快照** | 逐年 as-of 快照预计算（snapshot.py） | §8 | ⬜ |
| **财富曲线** | 账户/币种/公司/全家族合计 + USD 展示折算（账务本币/展示USD） | §7/§8 | ⬜ |
| **日历游标** | 全局日历 as-of 拖拽，全 App 联动 | §8 | ⬜ |
| **健康校验** | H1–H5 汇总 + 问题清单（health.py） | §10 | ⬜ |
| **增量重算** | 受影响起点向后传播（recompute.py）+ 提示 | §9 | ⬜ |
| **API** | 基础路由（entities/accounts/ledger/returns/fx/snapshots/overview/wealth） | §14 | ⬜ |
| **前端骨架** | React+Vite 10 屏骨架 + 全局日历 + 财富曲线 | §1 | ⬜ |

### Phase 1 —— P1（完整产品）

| 模块 | 功能 | 关键章节 | 状态 |
|---|---|---|---|
| **投资** | investment + alloc 派生数据、一年一投、R 级计息、年末赎回回银行、手动续投 | §19.1–19.4 | ⬜ |
| **投资** | 专款池（分币种）走账 + 服务层校验（as-of/R起始年/年度幂等/覆盖连锁拒绝） | §19.3 | ⬜ |
| **划拨/换汇** | 划拨(同币)/换汇(按年汇率) + 转出向后全链不破负拒绝 + 编年史同步 | §19.5 | ⬜ |
| **人物图谱** | 人—人/人—公司关系可视化（ECharts graph） | §1/§14 | ⬜ |
| **公司图谱** | 公司—公司关系 + 外部 API①② 导入（只增不减 + status） | §13 | ⬜ |
| **各国收益曲线** | return_curve（R1–R5）对比渲染 + 地区起始年下限 | §14 | ⬜ |
| **财务收支** | finance_entry 各类收入/支出，实体必填、以实体为中心浏览 | §5 | ⬜ |
| **统一搜索** | LLM+embedding RAG（omlx 本地）条目检索装配 + serve 后处理 | §18 | ⬜ |
| 四类 UI 改数据操作 | 统一模板：年份×池 + 后传重算 + 失败整体拒绝 + overlay 同步（§6.8 全清单） | §6.8/§19 | ⬜ |

### Phase 2 —— 事件 / 增强

| 模块 | 功能 | 关键章节 | 状态 |
|---|---|---|---|
| **事件·电影** | 电影事件导入 + 不关联 + 同币种 UI 手动关联（现金流入账户） | §19.6/§6.9 | ⬜ |
| **事件·股票** | holding_event(batch) + FIFO 成本 + 分红/卖出结算 + 被动抬升占比 | §19.6/§6.9 | ⬜ |
| **事件·股票** | 分拆 / 并购三形态（换股+现金/纯现金/纯换股）成本随链 | §19.6 | ⬜ |
| **事件·股票** | HP_CSC 重组链数值导入 → 依 §11.4 + H2 逐行验证 | §19.6 | ⬜ |
| **时间线/编年史 UI 编辑** | overlay 增改删、差异/重置回源/以源为最新 | §12/§6.4 | ⬜ |
| **文件 diff 回退** | 版本 diff → UI 决策「更新」/「回退」（DB+磁盘复原） | §11 | ⬜ |
| **导出** | markdown/CSV/报告 PDF（只导出不回写源） | §15 | ⬜ |
| **统一搜索增强** | 搜索数据质量达标后再评估是否需要 LLM 判文件兜底 | §18.5 | ⬜ |
