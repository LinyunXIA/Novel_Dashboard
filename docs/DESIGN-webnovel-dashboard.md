...
| 编号 | 模块 | 功能 | 关键章节 | 状态 |
|---|---|---|---|---|
| F-P1-01 | **投资** | investment + alloc 派生数据、一年一投、R 级计息、年末赎回回银行、手动续投｜core/invest.py，§19.1–19.4 全实现；审计修复：活期 2% 结息 core/demand.py 补齐（§19.2 注记）、批内同池 alloc 累计校验、赎回年末结算日 gate(409)、PATCH 重新锁定 422 | §19.1–19.4 | ✅ |
